...
| **A local account and a DSM identity become one account** | Driven against the running app with a throwaway local account: a token carrying its address (in a different case) was matched to it rather than given an account of its own, the identity row recorded `matched_by_email`, and **both doors then worked** — the real login form over HTTP signed it in with its password, and a second token found it by `sub` after the address had changed. The People page showed one row with both pills, its own username *and* the provider's subject; the account page said so in German, kept name and e-mail disabled, still offered "Neues Passwort setzen", and "Verknüpfung aufheben" took the identity off through the app's own dialog. The account was deleted afterwards. What is **not** covered: no real Synology token has ever been through this — see §2 |
...
| **…and for `v0.2.2`, with the notes written first** | 2026-08-09, run 31322888183, 2m44s. The other half of §8 exercised for the first time: a **draft** release created by hand before the tag was pushed, so the workflow attached its four assets to that rather than generating its own notes — `gh release view` finds a draft, which is what makes the branch in "Attach everything to the release" take the upload path. Then the shipped `docker-compose.yml` was downloaded back and read: `image: …:0.2.2` and `user: "${KITCHEN_UID:-1000}:${KITCHEN_GID:-1000}"`, with both names present in `env.example`. CI green on main in the same push (552 tests, 1m25s) |
...
- **Whether Synology's SSO Server sends an `email` claim the linking can use.**
  Accounts are now linked to a local one by address (§1), and every test of it
  hands the backend a dictionary. Two things nobody has seen from a real DSM:
  whether the address in the token is the one the household actually has in this
  app, and whether `email_verified` is sent at all — the code refuses only an
  explicit `false`, because requiring the claim would mean linking never
  happens on the builds this was written for. Worth reading one real token
